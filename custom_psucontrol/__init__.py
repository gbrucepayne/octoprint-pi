    ...
    def configure_gpio(self):
        ...
        if self.config['sensingMethod'] == 'GPIO':
            ...
            try:
                ...
                self._logger.info("Initial PSU state: {}".format(initial_state))
            except Exception:
                self._logger.exception(
        ...
        if self.config['switchingMethod'] == 'GPIO':
            ...
            # Retain the prior switch state if read from the sense state after restart
            if initial_state != 'ON':
    ...
    def turn_psu_off(self):
        if self.config['switchingMethod'] in ['GCODE', 'GPIO', 'SYSTEM', 'PLUGIN']:
            ...
            if self.config['disconnectOnPowerOff']:
                ...
                self._logger.warning("Removing power before disconnecting printer may crash Octoprint causing unexpected results")
