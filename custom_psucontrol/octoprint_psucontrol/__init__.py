...
class PSUControl(octoprint.plugin.StartupPlugin,
    ...
    """The Power Supply Unit control plugin class.
    """

    def __init__(self):
        # Enable type hinting for logger, printer
        self._logger: Logger = self._logger
        self._printer: PrinterInterface = self._printer
    ...
    def get_settings_defaults(self):
        return dict(
            ...
            autoOnTriggerGCodeCommands = (
                "G0,G1,G2,G3,G10,G11,G28,G29,G32,"
                "M104,M106,M109,M140,M190"
            ),
    ...
    def reload_settings(self):
        ...
        if (self.config['enablePseudoOnOff'] and
            self.config['switchingMethod'] == 'GCODE'):
            # Invalid configuration
            self._logger.warning("Pseudo On/Off cannot be used in conjunction"
                                 " with GCODE switching. Disabling.")
            self.config['enablePseudoOnOff'] = False
        self._autoOnTriggerGCodeCommandsArray = (
            self.config['autoOnTriggerGCodeCommands'].split(','))
        self._idleIgnoreCommandsArray = (
            self.config['idleIgnoreCommands'].split(','))

    def on_after_startup(self):
        if (self.config['switchingMethod'] == 'GPIO' or
            self.config['sensingMethod'] == 'GPIO'):
            # Setup GPIO
            self.configure_gpio()
        self._check_psu_state_thread = (
            threading.Thread(target=self._check_psu_state))
    ...
    def cleanup_gpio(self):
        for k, pin in self._configuredGPIOPins.items():
            self._logger.debug(f"Cleaning up {k} pin {pin.name}")
            try:
                pin.close()
            except Exception:
                self._logger.exception("Exception while cleaning up"
                                       f" {k} pin {pin.name}.")
        self._configuredGPIOPins = {}
    ...
    def configure_gpio(self):
        ...
        if self.config['sensingMethod'] == 'GPIO':
            sense_inverted = self.config['invertsenseGPIOPin']
            self._logger.info("Configuring PSU on/off sense"
                              f" on GPIO for pin {self.config['senseGPIOPin']}")
            if not SUPPORTS_LINE_BIAS:
                if self.config['senseGPIOPinPUD'] != '':
                    self._logger.warning("Kernel version 5.5 or greater required"
                                         " for GPIO bias. Using 'default'.")
                bias = "default"
            ...
            try:
                ...
                initial_sense = not pin.read() if sense_inverted else pin.read()
                initial_state = 'ON' if initial_sense else 'OFF'
                self._logger.info(f"Initial PSU state: {initial_state}")
            except Exception:
                self._logger.exception("Exception while setting up PSU sense"
                                       f" GPIO #{self.config['senseGPIOPin']}")
        # Setup PSU switch
        if self.config['switchingMethod'] == 'GPIO':
            self._logger.info("Configuring PSU On/Off"
                              f" using GPIO #{self.config['onoffGPIOPin']}")
            # default printer PSU state is OFF
            switch_inverted = self.config['invertonoffGPIOPin']
            if initial_state != 'ON':
                initial_output = 'high' if switch_inverted else 'low'
            else:
                initial_output = 'low' if switch_inverted else 'high'
            try:
                ...
                self._logger.exception("Exception while setting up PSU switch"
                                       f" GPIO #{self.config['onoffGPIOPin']}")
    ...
    def _check_psu_state(self):
        while True:
            ...
            if self.config['sensingMethod'] == 'GPIO':
                ...
                new_isPSUOn = False
                p = subprocess.Popen(self.config['senseSystemCommand'],
                                     shell=True)
                self._logger.debug("Sensing system command executed."
                                   f" PID={p.pid}, Command="
                                   f"{self.config['senseSystemCommand']}")
                while p.poll() is None:
                    time.sleep(0.1)
                r = p.returncode
                self._logger.debug(f"Sensing system command returned: {r}")
                if r == 0:
                    new_isPSUOn = True
                elif r == 1:
                    new_isPSUOn = False
                ...
                if p not in self._sub_plugins:
                    self._logger.error(f"Plugin {p} is configured for sensing"
                                       " but it is not registered.")
                elif not hasattr(self._sub_plugins[p], 'get_psu_state'):
                    self._logger.error(f"Plugin {p} is configured for sensing"
                                       " but get_psu_state is not defined.")
                else:
                ...
                self.isPSUOn = False
            self._logger.debug(f"isPSUOn: {self.isPSUOn}")
            if (old_isPSUOn != self.isPSUOn):
                self._logger.debug("PSU state changed,"
                                   "firing psu_state_changed event.")
    ...
    def _idle_poweroff(self):
        ...
        if self._printer.is_printing() or self._printer.is_paused():
            return
        self._logger.info("Idle timeout reached"
                          f" after {self.config['idleTimeout']} minute(s)."
                          " Turning heaters off prior to shutting off PSU.")
    ...
    def _wait_for_heaters(self):
        ...
        for heater, entry in heaters.items():
            ...
            try:
                ...
                continue
            if temp != 0:
                self._logger.info(f"Turning off heater: {heater}")
                self._skipIdleTimer = True
                self._printer.set_temperature(heater, 0)
                self._skipIdleTimer = False
            else:
                self._logger.debug(f"Heater {heater} already off.")
        while True:
            ...
            for heater, entry in heaters.items():
                ...
                try:
                    ...
                    # not a float for some reason, skip it
                    continue
                self._logger.debug(f"Heater {heater} = {temp}C")
            ...
            if highest_temp <= self.config['idleTimeoutWaitTemp']:
                self._waitForHeaters = False
                return True
            self._logger.info("Waiting for heaters before shutting off PSU"
                              f" ({', '.join(heaters_above_waittemp)})")
            time.sleep(5)

    def hook_gcode_queuing(self,
                           comm_instance,
                           phase,
                           cmd,
                           cmd_type,
        ...
        if self.config['enablePseudoOnOff']:
            if gcode == self.config['pseudoOnGCodeCommand']:
                ...
                comm_instance._log("PSUControl: ok")
                skipQueuing = True
        if (not self.isPSUOn and
            self.config['autoOn'] and
            (gcode in self._autoOnTriggerGCodeCommandsArray)):
            # Auto-on
            self._logger.info(f"Auto-On - Turning PSU On (Triggered by {gcode})")
            self.turn_psu_on()
        if (self.config['powerOffWhenIdle'] and
            self.isPSUOn and
            not self._skipIdleTimer):
            # Wait for headers
    ...
    def turn_psu_on(self):
        VALID_SWITCHING_METHODS = ['GCODE', 'GPIO', 'SYSTEM', 'PLUGIN']
        if self.config['switchingMethod'] in VALID_SWITCHING_METHODS:
            if self.config['switchingMethod'] == 'GCODE':
                gcode = self.config['onGCodeCommand']
                self._logger.debug(f"Switching PSU On Using GCODE: {gcode}")
                self._printer.commands(gcode)
            elif self.config['switchingMethod'] == 'SYSTEM':
                command = self.config['onSysCommand']
                self._logger.debug(f"Switching PSU On Using SYSTEM: {command}")
                p = subprocess.Popen(command, shell=True)
                self._logger.debug("On system command executed."
                                   f"PID={p.pid}, Command={command}")
                while p.poll() is None:
                    time.sleep(0.1)
                r = p.returncode
                self._logger.debug(f"On system command returned: {r}")
            elif self.config['switchingMethod'] == 'GPIO':
                self._logger.debug("Switching PSU On Using GPIO"
                                   f" #{self.config['onoffGPIOPin']}")
                ...
                self._logger.debug(f"Switching PSU On Using PLUGIN: {p}")
                if p not in self._sub_plugins:
                    self._logger.error(f"Plugin {p} is configured for switching"
                                       " but it is not registered.")
                    return
                elif not hasattr(self._sub_plugins[p], 'turn_psu_on'):
                    self._logger.error(f"Plugin {p} is configured for switching"
                                       " but turn_psu_on is not defined.")
                    return
                else:
            ...
            if (self.config['connectOnPowerOn'] and
                self._printer.is_closed_or_error()):
                # Connect printer
                self._logger.debug('Connecting to printer after PSU on')
    ...
    def turn_psu_off(self):
        ...
        if self.config['switchingMethod'] in VALID_SWITCHING_METHODS:
            ...
            if self.config['disconnectOnPowerOff']:
                self._logger.debug("Disconnecting before power off")
                self._printer.disconnect()
            if not self._printer.is_closed_or_error():
                self._logger.warning("Turning off PSU while connected"
                                     " may crash OptoPrint server")
            if self.config['switchingMethod'] == 'GCODE':
                gcode = self.config['offGCodeCommand']
                self._logger.debug(f"Switching PSU Off Using GCODE: {gcode}")
                self._printer.commands(gcode)
            elif self.config['switchingMethod'] == 'SYSTEM':
                command = self.config['offSysCommand']
                self._logger.debug(f"Switching PSU Off Using SYSTEM: {command}")
                p = subprocess.Popen(command, shell=True)
                self._logger.debug("Off system command executed."
                                   f"PID={p.pid}, Command={command}")
                while p.poll() is None:
                    time.sleep(0.1)
                r = p.returncode
                self._logger.debug(f"Off system command returned: {r}")
            elif self.config['switchingMethod'] == 'GPIO':
                self._logger.debug("Switching PSU Off Using GPIO"
                                   f" {self.config['onoffGPIOPin']}")
                ...
                p = self.config['switchingPlugin']
                self._logger.debug(f"Switching PSU Off Using PLUGIN: {p}")
                if p not in self._sub_plugins:
                    self._logger.error(f"Plugin {p} is configured for switching"
                                       " but it is not registered.")
                    return
                    ...
                    self._logger.error(f"Plugin {p} is configured for switching"
                                       " but turn_psu_off is not defined.")
                    return
                else:
    ...
    def on_event(self, event, payload):
        if event == Events.CLIENT_OPENED:
            ...
            self._logger.info("Firmware or communication error detected."
                              " Turning PSU Off")
            self.turn_psu_off()
            return
    ...
    def on_settings_save(self, data):
        if 'scripts_gcode_psucontrol_post_on' in data:
            ...
            uscript = u'' + script.replace("\r\n", "\n").replace("\r", "\n")
            self._settings.saveScript("gcode", "psucontrol_post_on", uscript)
        ...
        if 'scripts_gcode_psucontrol_pre_off' in data:
            ...
            uscript = u'' + script.replace("\r\n", "\n").replace("\r", "\n")
            self._settings.saveScript("gcode", "psucontrol_pre_off", uscript)
    ...
    def on_settings_migrate(self, target, current=None):
        ...
        if current < 2:
            ...
            if (cur_switchingMethod is not None and
                cur_switchingMethod == "COMMAND"):
                # Update switching method
                self._logger.info("Migrating Setting: switchingMethod=COMMAND"
                                  " -> switchingMethod=GCODE")
                self._settings.set(["switchingMethod"], "GCODE")
            cur_onCommand = self._settings.get(["onCommand"])
            if cur_onCommand is not None:
                self._logger.info(f"Migrating Setting: onCommand={cur_onCommand}"
                                  f" -> onGCodeCommand={cur_onCommand}")
            ...
            if cur_offCommand is not None:
                self._logger.info(f"Migrating Setting: offCommand={cur_offCommand}"
                                  f" -> offGCodeCommand={cur_offCommand}")
            ...
            if cur_autoOnCommands is not None:
                self._logger.info(f"Migrating Setting: autoOnCommands={cur_autoOnCommands}"
                                  f" -> autoOnTriggerGCodeCommands={cur_autoOnCommands}")
        ...
        if current < 3:
            ...
            if cur_enableSensing is not None and cur_enableSensing:
                self._logger.info("Migrating Setting: enableSensing=True"
                                  " -> sensingMethod=GPIO")
            ...
            if cur_switchingMethod == 'GPIO' or cur_sensingMethod == 'GPIO':
                if cur_GPIOMode == 'BOARD':
                    ...
                    try:
                        ...
                        self._logger.exception("Error importing RPi.GPIO."
                            " BOARD->BCM conversion will not occur")
                        _has_gpio = False
                    if cur_switchingMethod == 'GPIO' and _has_gpio:
                        p = _gpio_board_to_bcm(cur_onoffGPIOPin)
                        self._logger.info("Converting pin number from BOARD"
                            f" to BCM. onoffGPIOPin={cur_onoffGPIOPin}"
                            f" -> onoffGPIOPin={p}")
                        self._settings.set_int(["onoffGPIOPin"], p)
                    if cur_sensingMethod == 'GPIO' and _has_gpio:
                        p = _gpio_board_to_bcm(cur_senseGPIOPin)
                        self._logger.info("Converting pin number from BOARD"
                            f" to BCM. senseGPIOPin={cur_senseGPIOPin}"
                            f" -> senseGPIOPin={p}")
                ...
                if len(self._availableGPIODevices) > 0:
                    ...
                    gpio_device = self._availableGPIODevices[0]
                    self._logger.info("Setting GPIODevice to the first found."
                                      f" GPIODevice={gpio_device}")
                    self._settings.set(["GPIODevice"], gpio_device)
                else:
                    # GPIO was used for either but no GPIO devices exist. Reset to defaults.
                    self._logger.warning("No GPIO devices found."
                                         " Reverting switchingMethod"
                                         " and sensingMethod to defaults.")
                ...
                self._logger.info("Writing original GPIO related settings"
                                  " to psucontrol_rpigpio.")
